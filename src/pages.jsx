        ...
        <div className="booking-form-col">
          ...
          {status === 'success' ? (
            ...
            <div className="booking-form-wrap">
              <span className="booking-chip">Free · No commitment</span>
...
const PLANS = [
  {
    ...
    price: 'Free',
    period: '',
    desc: 'Your first analysis. No credit card, no setup — just upload your file.',
    color: '#F6F5F3',
    border: '#E8E6E2',
    accent: '#111111',
    features: [
  ...
  },
  {
    ...
    price: '€290',
    period: '/month',
    desc: 'All six modules for a full team. Monthly or weekly uploads.',
    color: '#1C1045',
    border: '#2D1B6B',
    accent: '#9B7FF5',
    features: [
  ...
  },
  {
    ...
    price: 'Custom',
    period: '',
    desc: 'For large distribution groups, multi-entity, or custom workflows.',
    color: '#F6F5F3',
    border: '#E8E6E2',
    accent: '#111111',
    features: [
...
const FAQ = [
  {
    ...
    a: 'No. Via works from standard CSV or Excel exports — the kind your ERP already produces. There is no connector, no API, no IT project. Just download and upload.',
  },
  {
    q: 'How long does the first analysis take?',
    a: 'Most teams have their first results in under 20 minutes. Upload your file, map a few columns, and Via does the rest.',
  },
  {
    q: 'Is my data secure?',
    a: 'Yes. Files are encrypted in transit and at rest. We never sell or share your data. You can delete your data at any time from your account.',
  },
  {
    q: 'What ERPs are compatible?',
    a: 'Any ERP that can export a CSV or Excel file — Sage, SAP, Cegid, Dynamics 365, Divalto, Infor, and others. If your ERP exports data, Via can read it.',
  },
  {
    q: 'Can I upgrade or cancel anytime?',
    a: 'Yes. Plans are monthly. You can upgrade, downgrade, or cancel at any time — no notice period required.',
  },
]

export function PricingPage({ onNav, onBooking }) {
  ...
  return (
    <PageShell onNav={onNav}>
      <div className="ip-hero" style={{ background: 'linear-gradient(135deg, #1C1045 0%, #2D1B6B 100%)' }}>
        <div className="container">
          <p className="ip-tag" style={{ color: '#9B7FF5' }}>Pricing</p>
          <h1 className="ip-title" style={{ color: '#fff' }}>Simple pricing.<br />Real value.</h1>
          <p className="ip-sub" style={{ color: 'rgba(255,255,255,0.7)' }}>Start free. Scale when you need it. No hidden fees, no long-term commitment.</p>
        </div>
      </div>
      ...
      <div className="container ip-body">
        <div className="ip-plans-grid">
          {PLANS.map((plan) => (
            <div
              key={plan.name}
              className={`ip-plan-card${plan.highlight ? ' highlight' : ''}`}
              style={{ background: plan.color, borderColor: plan.border }}
            >
              {plan.highlight && <div className="ip-plan-badge">Most popular</div>}
              <h3 className="ip-plan-name" style={{ color: plan.highlight ? '#fff' : plan.accent }}>{plan.name}</h3>
              <div className="ip-plan-price" style={{ color: plan.highlight ? '#9B7FF5' : plan.accent }}>
                {plan.price}<span className="ip-plan-period" style={{ color: plan.highlight ? 'rgba(255,255,255,0.5)' : '#888' }}>{plan.period}</span>
              </div>
              <p className="ip-plan-desc" style={{ color: plan.highlight ? 'rgba(255,255,255,0.6)' : '#666' }}>{plan.desc}</p>
              <ul className="ip-plan-features">
                {plan.features.map(f => (
                  <li key={f} style={{ color: plan.highlight ? 'rgba(255,255,255,0.85)' : '#333' }}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={plan.highlight ? '#9B7FF5' : '#3DAA3D'} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                    {f}
                  </li>
                ))}
              </ul>
              <button
                className={`ip-plan-cta${plan.highlight ? ' primary' : ''}`}
                onClick={onBooking}
                style={plan.highlight ? {} : { borderColor: plan.border, color: plan.accent }}
              >
                {plan.cta}
              </button>
            </div>
        ...
        <div className="ip-faq">
          ...
          {FAQ.map((item, i) => (
            <div key={i} className={`ip-faq-item${openFaq === i ? ' open' : ''}`}>
              <button className="ip-faq-q" onClick={() => setOpenFaq(openFaq === i ? null : i)}>
                {item.q}
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="ip-faq-chevron">
                  <polyline points="6 9 12 15 18 9"/>
                </svg>
              </button>
              {openFaq === i && <p className="ip-faq-a">{item.a}</p>}
            </div>
          ))}
        </div>

        <div className="ip-cta-block">
          <h2 className="ip-cta-title">Not sure which plan is right for you?</h2>
          <button className="ip-cta-btn" onClick={onBooking}>Talk to the team →</button>
        </div>
      </div>
    </PageShell>
  )
